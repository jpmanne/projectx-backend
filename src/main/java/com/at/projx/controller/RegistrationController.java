	...
	public ResponseEntity<Response> basicSetup(@Valid @RequestBody BasicSetupRequest basicSetupRequest) {
		...
		try {
			...
			if(organizations != null && !organizations.isEmpty()) {
				...
				return getInvalidInputResponseEntity(message);
			} 
			List<UserDetails> existingUsers = userRepository.getUserByEmail(basicSetupRequest.getWorkEmail().toLowerCase());
		...
		return getOKResponseEntity(message);
	}
}
